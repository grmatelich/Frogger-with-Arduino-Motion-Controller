...
from accelerometer import Controller
...
class PlayScreen(FScreen):
	...
	roll = self.game.controller.getAction()[0]
	pitch = self.game.controller.getAction()[1]    
	
	print(roll, pitch)
        #if event.type==KEYDOWN:
            #if event.key==K_LEFT:
 	if pitch < -30 and not self.isDown:
	    #set is down to true
            self.isDown = True
	...
	if pitch > 30 and not self.isDown:
	    ...
	    self.isDown = True
            self.frog.move_request((+1,0))
        #if event.key==K_UP:
        if roll < 155 and roll > 0 and not self.isDown:
	    #set is down to true
	    self.isDown = True
	...
	if abs(pitch) < 15 and abs(roll) > 165:
	    self.isDown = False
